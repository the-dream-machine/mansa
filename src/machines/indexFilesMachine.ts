...
export type IndexFilesMachineState =
	| {value: State.CHECKING_STATUS; context: MachineContext}
	...
	| {value: State.NEXT_PAGE; context: MachineContext};
...
>({
	...
	states: {
		[State.CHECKING_STATUS]: {
			invoke: {
				// Check if fishcake is already setup in repo
				src: async () => await fs.readJson('./.fishcake/config.json'),
				onDone: {
					target: State.NEXT_PAGE,
				},
				onError: {
					target: State.STARTING_DATABASE,
				},
			},
		},
		...
		[State.IDLE]: {
			on: {ENTER_PRESSED: State.FETCHING_FILE_PATHS},
		},
